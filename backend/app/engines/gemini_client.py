"""Gemini client with 3-key rotation for free tier — module-level singleton created on first use."""
...
class GeminiClient:
    ...
    def __init__(self):
        ...
        self.keys = [
            getattr(self.settings, f"GEMINI_API_KEY_{i}", "") for i in range(1, 4)
        ]
    ...
    def _rotate_key(self):
        ...
        for _ in range(len(self.keys)):
            ...
            if self.key_errors[self.current_key_index] < 3:
                break
    ...
    def get_usage_stats(self) -> Dict:
        ...
        return {
            "keys": len(self.keys), "current_index": self.current_key_index,
            "usage": self.key_usage, "errors": self.key_errors
        }
...
# Global instance — created on first import only if keys exist
_gemini_client: Optional[GeminiClient] = None


def get_gemini_client() -> GeminiClient:
    """Lazy-initialized global Gemini client."""
